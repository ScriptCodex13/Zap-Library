//#define ZAP_LIBRARY_MAIN2_CPP
#ifdef ZAP_LIBRARY_MAIN2_CPP
// Just a example
...
int main()
{
	...
	//class controller {
	//	auto mouse_callback = [&window, &camera, &currentPos, &sensitivity]() -> void
	//		{
	//			std::array<double, 2> newPos = window.GetMousePosition();
	//			float xoffset = (newPos[0] - currentPos[0]) * sensitivity;
	//			float yoffset = (currentPos[1] - newPos[1]) * sensitivity;
	//
	//			currentPos = newPos;
	//			camera.Rotate(xoffset, yoffset, 0.0f);
	//		};
	//};
	std::array<double, 2> currentPos = window.GetMousePosition();
	zap::util::CallbackInvoker mvccontroller ([&window, &camera, &currentPos, &sensitivity]()
		{
			std::array<double, 2> newPos = window.GetMousePosition();
			float xoffset = (newPos[0] - currentPos[0]) * sensitivity;
			float yoffset = (currentPos[1] - newPos[1]) * sensitivity;
